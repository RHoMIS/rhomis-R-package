...
calorie_calculations <- function(processed_data,
    ...
    if ("crop_calories" %in% names(calorie_conversions)& length(missing_columns)==0){
        ...
        extra_outputs$crop_calories_consumed_kcal <- map_to_wide_format(data = processed_data,name_column = "crop_name", column_prefixes = "crop_calories_consumed_kcal",
                                                                    types = "num")[[1]]
    ...
    if ("meat_calories" %in% names(calorie_conversions)& length(missing_columns)==0){
        ...
        extra_outputs$meat_calories_consumed_kcal <- map_to_wide_format(data = processed_data,name_column = "livestock_name", column_prefixes = "meat_calories_consumed_kcal",
                                                                    types = "num")[[1]]
    }
    ...
    if ("eggs_calories" %in% names(calorie_conversions)& length(missing_columns)==0){
        ...
        extra_outputs$eggs_calories_consumed_kcal <- map_to_wide_format(data = processed_data,name_column = "livestock_name", column_prefixes = "eggs_calories_consumed_kcal",
                                                                    types = "num")[[1]]
    ...
    if ("milk_calories" %in% names(calorie_conversions)& length(missing_columns)==0){
        ...
        extra_outputs$milk_calories_consumed_kcal <- map_to_wide_format(data = processed_data,name_column = "livestock_name", column_prefixes = "milk_calories_consumed_kcal",
                                                                    types = "num")[[1]]
    ...
    if ("honey_calories" %in% names(calorie_conversions) & length(missing_columns)==0){
        ...
        extra_outputs$bees_honey_calories_consumed_kcal <- map_to_wide_format(data = processed_data,
                                                                          name_column = "livestock_name",
