...
proportions_calculation <- function(data, use, use_column, prop_column, loop_number = NULL) {
    ...
    # then identify the looped column
    if (!is.null(loop_number)) {

        use_column <- paste0(use_column, "_", loop_number)
        prop_column <- paste0(prop_column, "_", loop_number)

        missing_columns <- check_columns_in_data(data,
                                                 individual_columns  = c(use_column,prop_column),
                                                 warning_message = "Cannot calculate proportion")
    ...
    # If there is no loop number provided in the arguments,
    # then just find the column
    ...
    # if there is at least one non-na
    # value, then split the uses
    # by space
    ...
    proportions_data[single_uses & is.na(proportions_data)] <- 1
